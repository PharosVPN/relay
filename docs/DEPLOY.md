...
The **embedded** relay needs no deployment — `helm` runs it in-process by
importing the `relay` package ([docs/HELM-INTEGRATION.md](HELM-INTEGRATION.md)).
...
nothing at runtime, and makes **no outbound connections** — `helm` dials in to
...
## 3. Enrol with helm (CSR-over-SSH)

Enrolment mirrors `buoy` node onboarding — CSR-over-SSH, no bootstrap token
(DESIGN §5, decision 14). `helm` automates the steps below over SSH; the
...
2. **Hand the CSR to helm.** `helm` signs it with the Fleet CA, assigning the
   ...
   does not self-assert these; see helm/BUILD.md, "Relay enrollment contract").

3. **Install the material helm returns** — owned by `beacon`:
...
| tunnel | `--tunnel-addr` (default `:8444`) | `helm` dialling in |
...
A healthy relay logs `relay serving caravel clients on …`; once `helm` dials
in it logs `helm tunnel connected`. `helm`'s admin UI shows the relay's tunnel
state.

## Certificate rotation

Relay certs are valid one year and `helm` rotates them (DESIGN §4) by pushing a
